...
> *You can find the latest released plugin version on [Gradle Plugin Portal](https://plugins.gradle.org/plugin/cn.dorck.code.guarder).*
...
- **Support automatic adaptation for both AGP 7.x and AGP 8.0+**
...
|:-----------------|-------------|------|
| `enable` | Whether to enable the plugin (default is `true`) | `boolean` |
...
#### AGP 7.x (Transform API)
- Uses `com.android.build.api.transform.Transform` API
- Processes directory and JAR inputs through `BaseTransform`
- Uses ASM `ClassVisitor` for bytecode manipulation

#### AGP 8.0+ (AsmClassVisitorFactory)
- Uses `com.android.build.api.instrumentation.AsmClassVisitorFactory` API
- Implements class filtering and visitor creation through `CodeGuardClassVisitorFactory`
- Registers factory using `AndroidComponentsExtension`
...
The project includes comprehensive unit tests covering core functionality modules:

```bash
# Run all tests
./gradlew :code-guard:test
```

Test coverage modules:
- `RandomCodeObfuscator` - Random code generator
- `AppCodeGuardConfig` - Global configuration management
- `CodeObfuscatorFactory` - Obfuscator factory
- `CodeGenerator` - Java code generator
- `ObfuscationClassVisitor` - ASM class visitor
- `Agp8Compat` - AGP version compatibility
