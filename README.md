...
| 可配置项                       | 说明                                       | 类型                |
|:---------------------------|------------------------------------------|-------------------|
| `enable`                   | 是否启用插件（默认为 `true`）                        | `boolean`         |
...
- 使用 `com.android.build.api.transform.Transform` API
- 通过 `BaseTransform` 处理目录和 JAR 输入
- 使用 ASM `ClassVisitor` 进行字节码操作

#### AGP 8.0+ (AsmClassVisitorFactory)
- 使用 `com.android.build.api.instrumentation.AsmClassVisitorFactory` API
- 通过 `CodeGuardClassVisitorFactory` 实现类过滤和访问器创建
- 使用 `AndroidComponentsExtension` 注册工厂
...
测试覆盖模块：
- `RandomCodeObfuscator` - 随机代码生成器
- `AppCodeGuardConfig` - 全局配置管理
- `CodeObfuscatorFactory` - 混淆器工厂
- `CodeGenerator` - Java 代码生成器
- `ObfuscationClassVisitor` - ASM 类访问器
- `Agp8Compat` - AGP 版本兼容性
